...
First, the WSS address of the designated API must be inputted into the apprpriate field in the .env file. For example, if the user decides to use an Ethereum Mainnet API from Infura, then the WSS address of that specific API must be used in order to view information about pairs on Uniswap that are on the Ethereum blockchain.
